...
*证明*：若 $\{H, \mu_\xi\} = 0$（对所有 $\xi \in \mathfrak{g}$，其中 $\mu_\xi = \langle\mu,\xi\rangle$），则 $\frac{d}{dt} \mu_\xi(\phi_H^t) = X_H(\mu_\xi) = \{H, \mu_\xi\} = 0$，故 $\mu_\xi$ 守恒。此为 Noether 定理的辛几何版本。$\blacksquare$

...
> **前置知识**：第176-179章（同调、同伦群、谱序列）

稳定同伦论（Stable Homotopy Theory）是现代代数拓扑的核心，它研究同伦论在"稳定范围"内的行为——即连续悬挂后的极限。

### 谱

一个**谱**（spectrum）$E$ 是一族带基点空间 $\{E_n\}_{n \geq 0}$ 连同同伦等价 $\Sigma E_n \to E_{n+1}$。谱的同伦范畴就是**稳定同伦范畴** $\mathcal{SH}$。

- 球谱 $\mathbb{S}$：$E_n = S^n$（球面的谱）
- Eilenberg-MacLane 谱 $HA$：$HA_n = K(A,n)$
- 拓扑 K-理论谱 $KU$，$KO$
- 配边谱 $MU$，$MO$，$MSpin$

$\mathcal{SH}$ 是一个**三角范畴**并具有对称幺半结构，其单位是 $\mathbb{S}$。

### 色展理论（Chromatic Homotopy Theory）

色展理论（由 Quillen、Morava、Ravenel 奠基）提供了一种对稳定同伦范畴 $\mathcal{SH}$ 进行"分层"的纲领。

**核心思想**：通过形式群定律的高度（height）在质数 $p$ 处组织理论。

| height | 色展层 | 典型上同调理论 |
|--------|--------|---------------|
| 0 | 有理同伦 | $H\mathbb{Q}$ |
| 1 | K-理论局部 | $KU_p$（p-完备复 K-理论）|
| 2 | 椭圆层 | 椭圆上同调 $TMF$ |
| ≥2 | Morava K-理论层 | Morava $K(n)$ |

**Morava K-理论** $K(n)$ 是色展理论的"探测工具"：它的系数环为 $\mathbb{F}_p[v_n^{\pm 1}]$（其中 $|v_n| = 2(p^n-1)$），并具有 Künneth 定理。

### Ravenel 猜想

Doug Ravenel 提出了一系列关于球面同伦群和色展过滤的猜想，其中最著名的是**幂零猜想**和**望远镜猜想**——它们塑造了 1980 年代后的稳定同伦论方向。几乎所有 Ravenel 猜想现已被证明（通过 Hopkins、Smith、Devinatz 等人的工作）。

### 色展红移

在色展层之间上升（增加 height）时，代数 K-理论展现出"红移"现象：将 height $n$ 的对象送入 height $n+1$ 的环境。这一现象已在多种背景中得到证实（Thomason, Ausoni-Rognes），并猜测也与代数 K-理论的"红移猜想"相容。

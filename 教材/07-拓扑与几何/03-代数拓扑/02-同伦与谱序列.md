...
*证明概要*：$\pi_3(S^2) \cong \mathbb{Z}$ 由 Hopf 纤维化 $S^1 \to S^3 \to S^2$ 的同伦正合序列和 $\pi_3(S^3) \cong \mathbb{Z}$ 给出。$\pi_4(S^3) \cong \mathbb{Z}/2$ 由 Hopf 不变量和 Freudenthal 悬挂给出（见定理 80.5）。$\pi_4(S^2) \cong \mathbb{Z}/2$ 由 Hopf 纤维化正合序列推出。其他值通过稳定同伦群和 Serre 谱序列迭代计算得到，涉及经典的结果（Serre, Toda 等）。$\blacksquare$
...
*证明概要*：构建映射柱 $M_f = (X \times I) \sqcup Y / \sim$，其中 $(x,1) \sim f(x)$。$X$ 嵌入为 $X \times \{0\}$。需证明 $X$ 是 $M_f$ 的强形变收缩核。由同伦群同构和相对 Hurewicz 定理，知 $\pi_n(M_f, X) = 0$ 对所有 $n$ 成立。因 $(M_f, X)$ 是 CW 对，存在胞腔逐个收缩的论证，证明 $X \hookrightarrow M_f$ 是同伦等价。又 $Y \hookrightarrow M_f$ 也是同伦等价。故 $X \simeq Y$。$\blacksquare$
...
*证明概要*：$\Sigma X$ 可视为两个锥 $\operatorname{Cone}_+(X)$ 和 $\operatorname{Cone}_-(X)$ 沿 $X$ 粘合。悬挂映射 $E[f]$ 由在每个锥上分别定义 $f$ 得到。对 $S^k \to \Sigma X$ 的映射，若 $k \leq 2n-2$，则可用一般位置的组合论证（或光滑逼近）将映射同伦形变为落在 "pjoin" 分解中的映射。该论证（本质上是 Blakers-Massey 定理）表明 $\Sigma X$ 的 $(2n-1)$-骨架同伦等价于 $\Sigma (X^{(n-1)})$ 与额外胞腔的楔积，而 $E$ 在所示范围内为同构。$\blacksquare$
...
*证明概要*：对 $B$ 作 CW 骨架滤过，拉回得 $E$ 的上链复形滤过。应用上链复形的谱序列构造定理（定理 80.1 的对偶版本），$E_1^{p,q}$ 项由 $F$ 在 $E$ 中限制的上同调决定。因纤维化在 $B$ 胞腔上局部平凡，$E_2^{p,q} \cong H^p(B; H^q(F))$。乘法结构来自上链复形的杯积与滤过的相容性。$\blacksquare$
...
*证明概要*：路径空间 $PK(\mathbb{Z}, n)$ 可缩，故纤维化给出同伦等价 $PK \simeq *$。Serre 谱序列 $E_2^{p,q} = H^p(K(\mathbb{Z}, n); H^q(K(\mathbb{Z}, n-1))) \Rightarrow H^{p+q}(*)$ 必须收敛到零。这强制 $E_\infty$ 项全部为零，从而通过微分递推确定 $H^*(K(\mathbb{Z}, n))$。例如 $n=2$ 时唯一非零生存元是 $x = \iota_2 \in H^2$，且 $d_3(x^n) = n x^{n-1} \otimes y$ 等方程强制 $H^*(K(\mathbb{Z},2)) = \mathbb{Z}[x]$。这是 Cartan-Serre 方法的核心。$\blacksquare$

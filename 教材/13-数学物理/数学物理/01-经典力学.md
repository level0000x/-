...
**证明**：由Hamilton原理$\delta S=\delta\int L dt=0$。Legendre变换$H(q,p)=\sup_{\dot{q}}(p\dot{q}-L(q,\dot{q}))$。若$L$关于$\dot{q}$严格凸，则$p=\partial L/\partial\dot{q}$可逆，$\dot{q}=\partial H/\partial p$。Euler-Lagrange方程$\frac{d}{dt}\frac{\partial L}{\partial\dot{q}}=\frac{\partial L}{\partial q}$等价于$\dot{p}=-\partial H/\partial q$（由$p$的定义和Legendre变换性质）。两组方程$\dot{q}=\partial H/\partial p$，$\dot{p}=-\partial H/\partial q$即为Hamilton正则方程。$\blacksquare$

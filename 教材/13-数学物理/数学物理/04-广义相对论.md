...
**证明**（导出）：从Einstein-Hilbert作用量$S[g]=\frac{1}{16\pi G}\int_M R\sqrt{-g}\,d^4x+S_{\text{matter}}$出发。度规变分$\delta g^{\mu\nu}$给出$\delta S=\frac{1}{16\pi G}\int(G_{\mu\nu}-8\pi G T_{\mu\nu})\delta g^{\mu\nu}\sqrt{-g}\,d^4x$，其中$G_{\mu\nu}=R_{\mu\nu}-\frac{1}{2}Rg_{\mu\nu}$为Einstein张量。$\delta S=0$对所有$\delta g^{\mu\nu}$成立，由变分法基本引理得$G_{\mu\nu}=8\pi G T_{\mu\nu}$。Bianchi恒等式$\nabla^\mu G_{\mu\nu}=0$蕴含能量动量守恒$\nabla^\mu T_{\mu\nu}=0$。$\blacksquare$
...
**定理 151.8**（正质量定理 / Schoen-Yau, Witten）：若时空满足主导能量条件，则 $M_{ADM} \geq 0$，且 $M_{AD

**证明**（Witten旋量证明）：设$\Sigma$为渐近平坦类空超曲面。构造Witten旋量$\psi$满足Dirac-Witten方程$\gamma^i\nabla_i\psi=0$，渐近条件$\psi\to\psi_0$（常数旋量）。积分恒等式：$\int_{\partial\Sigma_\infty}\langle\psi,\gamma^i\nabla_i\psi\rangle dS_i=\int_\Sigma(|\nabla\psi|^2+8\pi G T_{\mu\nu}n^\mu n^\nu|\psi|^2)dV$。渐近分析给出左边$=4\pi G M_{\text{ADM}}|\psi_0|^2$。主导能量条件$T_{\mu\nu}n^\mu n^\nu\ge 0$保证右边非负，故$M_{\text{ADM}}\ge 0$。$M_{\text{ADM}}=0$时$\nabla\psi=0$，$\Sigma$为$\mathbb{R}^3$。$\blacksquare$M} = 0$ 当且仅当时空是 Minkowski 时空。

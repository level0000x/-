...
> **前置知识**：第217-220章（代数几何）、第225-229章（范畴论与同调代数）

导出代数几何（Derived Algebraic Geometry, DAG）是将同伦论方法引入代数几何的现代理论，由 Toën、Vezzosi 和 Lurie 等人系统发展。

### 动机

经典代数几何中，研究方程 $f(x) = 0$ 的解空间（簇/概形）时遇到如下限制：

1. **交理论问题**：两个子簇沿非横截方向相交时，交的维数超出预期
2. **形变问题**：要理解形变空间，需要切复形（cotangent complex）的完整同伦型
3. **模空间问题**：模空间常常有奇异性，需要在导出意义上理解

### 核心思想

DAG 的关键思想：用**单纯环**（或 dg 代数 / $E_\infty$-环谱）替换经典结构层 $\mathcal{O}_X$ 中的交换环。确切地说：

- **导出概形** = 拓扑空间 $X$ + 交换单纯环（或 cdga）构成的结构层 $\mathcal{O}_X^\bullet$
- 其"基环"（$H^0$）恢复经典概形

### 切复形与余切复形

在 DAG 中，$\Omega^1$ 和切空间的概念被替换为**余切复形** $\mathbb{L}$ 和**切复形** $\mathbb{T}$，它们作为同伦对象而非单纯层/模。

- 光滑态射：$\mathbb{L}$ 作为完美复形集中在度 0
- 一般态射：$\mathbb{L}$ 承载负度中的上同调信息

### Lurie 的框架

Jacob Lurie 在 *Derived Algebraic Geometry* 系列论文中以 **$\infty$-范畴**为基础建立 DAG。其核心构造：

- **谱概形**（spectral scheme）：由 $E_\infty$-环谱构成
- **导出叠**（derived stack）：取值于 $\infty$-广群而不是集合的函子
- **形式模问题**：导出形变理论中的形式模问题自动由 dg Lie 代数编码

### 应用

- **导出交理论**：Serre 交重数公式在导出意义下有精确的推广
- **几何 Langlands**：$\mathcal{D}$-模理论在导出代数几何中获得自然框架
- **导出模空间**：向量丛的模叠 $\operatorname{Bun}_G(X)$ 的导出结构承载自守形式的深层信息

---

## 高阶范畴论导引：$\infty$-范畴

> **前置知识**：第225-229章（范畴论基础）

高阶范畴论（Higher Category Theory）推广了经典范畴论，允许在对象之间存在"态射之间的态射"（2-态射）、"2-态射之间的态射"（3-态射）等等，无限制地向上堆叠。

### 严格n-范畴与弱n-范畴

- **严格 2-范畴**：$\operatorname{Hom}(X,Y)$ 本身是一个范畴（态射的自然变换）
- **双范畴**（bicategory）：合成律仅在同构意义下成立（弱化版本）
- **弱 n-范畴**：所有高阶层面的合成律仅在某种高阶等价意义下成立

弱 n-范畴的理论远比严格情形复杂。主要模型包括：
- Batanin 的 $\omega$-operad 方法
- Rezk 的 $\Theta_n$-空间
- Barwick 和 Schommer-Pries 的公理刻画

### $\infty$-范畴的模型

$\infty$-范畴（即 $(\infty,1)$-范畴）是弱化的范畴，其中 1-态射可在同伦意义上合成，而所有更高态射都可逆。三种主要的实现模型：

| 模型 | 构造 | 主要文献 |
|------|------|---------|
| 拟范畴（quasi-category） | 满足内 Kan 扩张条件的单纯集 | Joyal, Lurie |
| 完备 Segal 空间 | 满足完备性条件的双单纯集 | Rezk |
| 单纯富范畴 | 纤维于单纯集范畴的范畴 | Bergner |

这三种模型是 Quillen 等价的。

### $\infty$-范畴中的核心结果

- **$\infty$-范畴中的伴随函子定理**（Lurie）
- **$\infty$-意象理论**：$(\infty,1)$-意象是意象概念的 $(\infty,1)$-范畴化，具有万有性质和 $(\infty,1)$-层语义
- **稳定 $\infty$-范畴**：稳定 $(\infty,1)$-范畴是导出范畴和谱范畴的自然推广，支撑着同调代数的完整理论

### 应用

- 导出代数几何的完整框架（Lurie's DAG 系列）
- 因子化同调与拓扑量子场论的分类
- 色展同伦论（chromatic homotopy theory）中的无穷层

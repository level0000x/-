...
**定义 83.17**（拟凝聚层与凝聚层）：概形 $X$ 上的 $\mathcal{O}_X$-模层 $\mathcal{F}$ 称为**拟凝聚层**，若局部地可表示为自由层的余核。若进一步局部地可表示为有限秩自由层的余核，则称为**凝聚层**。拟凝聚层范畴 $\operatorname{QCoh}(X)$ 是 Grothendieck 引入的 Abel 范畴，其上同调理论是代数几何的核心工具。

**定义 83.18**（层上同调）：设 $X$ 为概形，$\mathcal{F}$ 为 $\mathcal{O}_X$-模层。整体截面函子 $\Gamma(X, -)$ 是左正合函子，其右导出函子 $H^i(X, \mathcal{F})$ 称为 $X$ 的**层上同调群**。对于仿射概形 $X = \operatorname{Spec} A$ 和拟凝聚层 $\mathcal{F} = \tilde{M}$（$M$ 为 $A$-模），Serre 定理断言 $H^i(X, \mathcal{F}) = 0$ 对所有 $i > 0$ 成立——即仿射概形上层上同调平凡。

**定理 83.4（Serre 对偶）**：设 $X$ 为域 $k$ 上的 $n$ 维光滑射影概形，$\omega_X = \Omega_{X/k}^n$ 为典范层。则对任意凝聚层 $\mathcal{F}$，存在自然同构 $H^i(X, \mathcal{F}) \cong \operatorname{Ext}^{n-i}(\mathcal{F}, \omega_X)^*$（有限维 $k$-向量空间意义下的对偶）。Serre 对偶是代数几何版本的 Poincaré 对偶，是 Riemann-Roch 定理和相交理论的核心支柱。

**证明**（框架）：构造对偶化复形 $\omega_X^\bullet$（在光滑情形下拟同构于 $\omega_X[n]$）。由 Grothendieck 对偶理论，$\operatorname{RHom}(\mathcal{F}, \omega_X^\bullet)$ 的对偶给出 $R\Gamma(X, \mathcal{F})$。在光滑射影情形下 $\omega_X^\bullet = \omega_X[n]$，取上同调即得所证。$\blacksquare$

### 83.6 平坦性与下降理论

**定义 83.19**（平坦态射）：概形态射 $f: X \to Y$ 称为**平坦的**，若对所有 $x \in X$，局部环同态 $\mathcal{O}_{Y, f(x)} \to \mathcal{O}_{X, x}$ 是平坦的（即 $\mathcal{O}_{X, x}$ 作为 $\mathcal{O}_{Y, f(x)}$-模是平坦的）。平坦性是代数几何中"连续变化"的代数化——直觉上，平坦族中各纤维的"尺寸"连续变化（如 Hilbert 多项式恒定）。

**定理 83.5（平坦下降 / fpqc 下降）**：设 $f: Y \to X$ 为忠实平坦拟紧态射（fpqc 覆盖）。则 $X$ 上的拟凝聚层范畴等价于带有下降数据的 $Y$ 上的拟凝聚层范畴。这是 Grothendieck 下降理论的核心：概形上的几何对象可由其在一个平坦覆盖上的数据"粘合"得到，条件是这些数据在"重叠"部分满足相容性。

这一理论使得在代数几何中"局部地"（在平坦拓扑而非仅 Zariski 拓扑意义下）工作成为可能，是平展拓扑、fppf 拓扑以及更一般的 Grothendieck 拓扑的起源。$\blacksquare$
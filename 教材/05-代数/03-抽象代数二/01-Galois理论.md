...
(1) 任意有限可分扩张是单扩张（本原元素定理）：存在 $\alpha \in E$ 使 $E = F(\alpha)$。则 $|G| = [E : F]$：因为 $G$ 中自同构由 $\alpha$ 的像决定，$\alpha$ 可映到其最小多项式 $m_\alpha(x)$ 的任意根，而 Galois 扩张中 $m_\alpha(x)$ 的所有根都在 $E$ 中且互异，故 $|G| = \deg m_\alpha = [E : F]$。

(2) Artin 引理：若 $H = \{\sigma_1, \ldots, \sigma_n\}$ 是 $E$ 的自同构构成的有限群，且 $\sum_{i=1}^n \alpha_i \sigma_i(x) = 0$ 对所有 $x \in E$ 成立，则 $\alpha_i = 0$。证明：假设存在非平凡关系，取项数最少者 $\sum_{i=1}^m \alpha_i \sigma_i(x) = 0$（$\alpha_i \neq 0$）。由 $\sigma_1 \neq \sigma_m$，存在 $y \in E$ 使 $\sigma_1(y) \neq \sigma_m(y)$。将关系应用于 $yx$ 得 $\sum \alpha_i \sigma_i(y) \sigma_i(x) = 0$；乘以 $\sigma_m(y)$ 减去原关系乘以 $\sigma_1(y)$，消去 $\sigma_1$ 项，得项数更少的非平凡关系，矛盾。由 Artin 引理得 $[E : E^H] \geq |H|$；又 $H \subseteq \operatorname{Gal}(E/E^H)$ 且 $|\operatorname{Gal}(E/E^H)| \leq [E : E^H]$，故 $[E : E^H] = |H|$。

(3) 由 (1) 和 (2) 得：$E^{\operatorname{Gal}(E/K)} = K$（因 $[E : K] = |\operatorname{Gal}(E/K)|$，且 $E^{\operatorname{Gal}(E/K)}$ 含 $K$ 且次数相同），$\operatorname{Gal}(E/E^H) = H$（因 $|H| = [E : E^H]$ 且 $H \subseteq \operatorname{Gal}(E/E^H)$）。从而建立中间域与子群之间的一一对应。

(4) 正规性条件：$K/F$ 正规等价于 $\sigma(K) = K$ 对所有 $\sigma \in G$ 成立，等价于 $\sigma \operatorname{Gal}(E/K) \sigma^{-1} = \operatorname{Gal}(E/\sigma(K))$，故 $\operatorname{Gal}(E/K) \trianglelefteq G$ 当且仅当 $\sigma(K) = K$。$\blacksquare$
...
**证明**：($\Rightarrow$) 若 $f$ 可用根式求解，则存在根式扩张塔 $F = F_0 \subseteq F_1 \subseteq \cdots \subseteq F_m$ 包含 $f$ 的分裂域 $E$。每个 $F_{i+1}/F_i$ 是添加 $p$ 次单位根或 $p$ 次方根（$p$ 为素数），对应的 Galois 群是循环群 $C_p$（或平凡群），故为 Abel 群。取 $E$ 为包含 $E$ 和所有必要单位根的 Galois 闭包，则 $\operatorname{Gal}(E/F)$ 的子群链 $\operatorname{Gal}(E/F) = G_0 \supseteq G_1 \supseteq \cdots \supseteq G_m = \{\operatorname{id}\}$ 对应到扩张塔，且 $G_i / G_{i+1} \leq C_p$ 为 Abel 群。由 Galois 对应，$\operatorname{Gal}(f/F) = \operatorname{Gal}(E/F)$ 是 $G := \operatorname{Gal}(E/F)$ 的商群，而 $G$ 可解，故 $\operatorname{Gal}(f/F)$ 可解。

($\Leftarrow$) 若 $\operatorname{Gal}(f/F)$ 可解，则存在正规列 $G = G_0 \trianglerighteq G_1 \trianglerighteq \cdots \trianglerighteq G_m = \{e\}$，商群 $G_{i-1}/G_i \cong C_{p_i}$（素数阶循环群）。由 Galois 对应，这对应于一列中间域 $F = K_0 \subseteq K_1 \subseteq \cdots \subseteq K_m = E$，其中 $K_i/K_{i-1}$ 是 Galois 扩张，Galois 群为 $C_{p_i}$。添加 $p_i$ 次单位根后，$K_i/K_{i-1}$ 成为根式扩张（因 $C_{p_i}$ 扩张在包含单位根时由 $p_i$ 次方根生成）。故 $f$ 可用根式求解。$\blacksquare$
...
**证明**：尺规作图的基本步骤是：(1) 作过两点的直线；(2) 作以给定点为圆心、给定两点间距离为半径的圆。新点由线性方程和二次方程的解给出。因此，从 $\mathbb{Q}$ 出发，每一步的坐标都在一个二次扩张中。反之，若存在二次扩张塔，则每个中间域 $K_i = K_{i-1}(\sqrt{a_i})$，$\sqrt{a_i}$ 可用尺规作出（由相似三角形构造平方根）。归纳得 $\alpha$ 可用尺规作出。$\blacksquare$

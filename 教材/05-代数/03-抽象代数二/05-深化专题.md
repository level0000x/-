...
**证明框架**（Kemer的理论）：
1. **表示论归约**：将自由代数视为对称群 $\mathfrak{S}_n$ 的表示空间。T-理想对应 $\mathfrak{S}_n$-子表示的序列。
2. **有限基条件**：由 $\mathfrak{S}_n$ 表示的全可约性（特征零），$k\langle X \rangle$ 的多线性部分作为 $\mathfrak{S}_n$-模完全可约。T-理想的有限基等价于对应的 $\mathfrak{S}_n$-表示序列满足某种 Noether 性质。
3. **Grassmann包络技巧**：Kemer 引入 Grassmann 代数张量积的构造，将任意 PI 代数与有限维（超）代数相关联，从而将 Specht 问题归约到有限维代数的情形。
4. **终结**：有限维代数的 T-理想由幂零指标和 Wedderburn-Malcev 分解控制，经有限步后稳定，故有限基。$\blacksquare$

...
*证明*：线性化得 Jacobian $J = \begin{pmatrix} -\gamma_m & -f'(p^*) \\ \beta & -\gamma_p \end{pmatrix}$，其中 $f'(p^*) < 0$。特征方程 $\lambda^2 - \operatorname{Tr}(J)\lambda + \det(J) = 0$。$\operatorname{Tr}(J) = -(\gamma_m+\gamma_p) < 0$ 恒负，但 $n$ 增大使 $|f'(p^*)|$ 增加，行列式增大，从而特征值虚部增大并最终穿过虚轴。当 $\operatorname{Tr}(J) = 0$ 时产生纯虚特征值，满足 Hopf 分岔条件。第一 Lyapunov 系数通常为负（$n > 8$），故分岔为超临界，产生稳定极限环。$\blacksquare$
...
*证明概略*：考虑相邻状态间 Hamming 距离 $d(t)$ 的演化。$K=1$ 时，每个节点仅受单一输入，动力学可分解为独立环路，$d(t)$ 随时间衰减至 0。$K \geq 3$ 时，布尔函数的高度非线性使小扰动以正概率传播，$d(t)$ 期望指数增长。$K=2$ 为临界点：此时每个节点的两个输入恰好提供边缘信息，扰动既不发散也不消失，产生"混沌边缘"的丰富动力学——Lyapunov 指数在临界点附近为零。$\blacksquare$
...
*证明*：化学主方程的无穷小生成元表明，从状态 $\mathbf{x}$ 出发到下一次跳跃的时间为指数分布 $\operatorname{Exp}(a_0(\mathbf{x}))$（$R$ 个独立指数分布竞争的最小值）。由指数分布的无记忆性，反应 $j$ 被选中的概率恰为 $a_j/a_0$。此抽样过程等价于 Markov 过程的转移核，故 Gillespie 算法产生精确的随机模拟，不引入任何近似误差。$\blacksquare$
...
*证明*：令 $\mathbf{y}_n = \mathbf{x}_n - \mathbf{x}^*$，则 $\mathbf{y}_{n+1} \approx Df(\mathbf{x}^*)\mathbf{y}_n$。设 $Df(\mathbf{x}^*)$ 的 Jordan 分解为 $PJP^{-1}$，则 $\mathbf{y}_n \approx PJ^n P^{-1}\mathbf{y}_0$。若所有 $|\lambda| < 1$，则 $\|J^n\| \leq C \rho^n$（$\rho < 1$），故 $\|\mathbf{y}_n\| \to 0$（渐近稳定）。若存在 $|\lambda| > 1$，沿对应特征方向 $\|\mathbf{y}_n\| \to \infty$（不稳定）。$\blacksquare$
...
*证明*：(1) 构造序列 $\mathbf{s}$ 依次包含所有有限长二进制串，其轨道在 $\Sigma_2$ 中稠密。(2) 有限长周期序列如 $010101\ldots$ 在度量 $d$ 下稠密。(3) 对任意 $\mathbf{s}$ 和 $\varepsilon > 0$，取 $\mathbf{t}$ 与 $\mathbf{s}$ 前 $N$ 位相同但第 $N+1$ 位不同（取 $2^{-N} < \varepsilon$），则 $\sigma^N(\mathbf{t})$ 和 $\sigma^N(\mathbf{s})$ 的首位距离 $\geq 1/2$。$\blacksquare$

**定理 137.6**（Logistic 映射与符号动力学的联系）：$r = 4$ 时，Logistic 映射 $f(x) = 4x(1-x)$ 拓扑共轭于移位映射。具体地，半共轭由 $h(\theta) = \sin^2(\pi\theta)$ 给出（通过帐篷映射）。

*证明*：考虑帐篷映射 $T(y) = 2y$（$0 \le y \le 1/2$），$T(y) = 2(1-y)$（$1/2 < y \le 1$）。$T$ 通过二进制展开 $\{0,1\}$ 拓扑共轭于移位映射。变换 $h(y) = \sin^2(\pi y/2)$ 将 $T$ 共轭到 Logistic 映射：$f \circ h = h \circ T$（利用三角恒等式 $\sin^2(\pi y) = 4\sin^2(\pi y/2)(1-\sin^2(\pi y/2))$）。故 $f$ 通过帐篷映射间接拓扑共轭于移位映射。$\blacksquare$
...
*证明*：沿轨线 $\frac{d}{dt}V(\phi_t(x)) = \nabla V \cdot X = \dot{V}(\phi_t(x)) \leq 0$，故 $V$ 不增。设 $B_\varepsilon \subset U$，$m = \min_{\|y-x^*\|=\varepsilon} V(y) > 0$。由 $V$ 连续性取 $\delta$ 使 $\|x-x^*\|<\delta \Rightarrow V(x)<m$。因 $V$ 不增，从 $\delta$-邻域出发的轨线永不超过 $\varepsilon$-邻域（Lyapunov 稳定）。若 $\dot{V}<0$，由 LaSalle 不变原理，轨线的 $\omega$-极限集含于 $\{x: \dot{V}(x)=0\}$ 的最大不变子集。该集仅含 $x^*$，故渐近稳定。$\blacksquare$
...
*证明*：设 $K$ 为正向不变紧集且不含平衡点。对任意 $x \in K$，其 $\omega(x)$ 是非空紧不变集且含于 $K$。因 $K$ 不含平衡点，由 Poincaré-Bendixson 定理，$\omega(x)$ 必为周期轨道。故 $K$ 中至少存在一个周期轨道。$\blacksquare$
...
*证明概略*：在 $\mu_0$ 附近将系统约化到二维中心流形，引入极坐标 $(r,\theta)$ 得 $\dot{r} = \alpha'(\mu_0)(\mu-\mu_0) r + a r^3 + \cdots$，$\dot{\theta} = \omega(\mu_0) + \cdots$。横截条件 $\alpha' \neq 0$ 保证当 $\mu$ 穿过 $\mu_0$ 时径向平衡点 $r=0$ 的分岔。若 $a < 0$（超临界），$\mu > \mu_0$ 时产生稳定极限环 $r \approx \sqrt{\alpha'(\mu_0)(\mu-\mu_0)/(-a)}$。$\blacksquare$
...
*证明概略*：设 $f$ 满足条件 (2)-(3)。取 $\delta_0 = \operatorname{diam}(X)/4 > 0$。由稠密周期点，存在两个不同周期轨道距离 $\geq 4\delta_0$。利用拓扑传递性可证：对任意 $x$ 和 $\varepsilon > 0$，存在 $y$ 满足 $d(x,y) < \varepsilon$ 且轨道在有限步后分歧超过 $\delta_0$——此即敏感依赖性。$\blacksquare$
...
*证明概略*：$k$ 维体积元的膨胀率为 $e^{(\lambda_1+\cdots+\lambda_k)t}$，需收缩方向补偿以使体积有界。整数部分 $k$ 对应正扩张维度。分数部分 $(\sum_{i=1}^k \lambda_i)/|\lambda_{k+1}|$ 来自 $k$ 个扩张方向与 $|\lambda_{k+1}|$ 收缩率的平衡 —— 体积元扩张被 $k+1$ 维收缩抵消时维数为 $k$ 加分数插值。Lorenz 系统指数约 $(2.16, 0, -32.4)$，得 $2 + 2.16/32.4 \approx 2.06$。$\blacksquare$
...
*证明*：由 Birkhoff 遍历定理，若 $T$ 遍历，极限函数 $\bar{f}$ 几乎处处为常数且 $\int \bar{f} d\mu = \int f d\mu$。故 $\bar{f}(x) = \int f d\mu$ ae，即 $\lim_{n\to\infty} \frac{1}{n}\sum_{k=0}^{n-1} f(T^k x) = \int f d\mu$ 对几乎所有 $x$ 成立。$\blacksquare$
...
*证明*：(1 ⇒ 2) 设 $f$ 为不变可测函数，$A_t = \{f > t\}$ 为不变集，故 $\mu(A_t) \in \{0,1\}$，$f$ 几乎处处为常数。(2 ⇒ 3) 由 Birkhoff 遍历定理，$\mathbf{1}_{A}$ 的时间平均 ae 等于其空间平均 $\mu(A)$。乘以 $\mathbf{1}_{B}$ 求积分并交换极限即得。(3 ⇒ 1) 取 $A$ 为不变集，$B = A$，条件(3) 给出 $\mu(A) = \mu(A)^2$，故 $\mu(A) \in \{0,1\}$。$\blacksquare$
...
*证明*：在 $L^2(\mu) = \mathbb{C} \oplus \mathcal{H}_0$ 分解下，弱混合等价于对所有 $f,g \in \mathcal{H}_0$，$\frac{1}{n}\sum_{k=0}^{n-1} |\langle U_T^k f, g\rangle| \to 0$。由 Koopman 算子的谱定理，$U_T$ 在 $\mathcal{H}_0$ 上有连续谱当且仅当对任意 $f,g$，相关序列 $\langle U_T^k f, g\rangle$ 的 Cesaro 平方平均趋于零——这正是弱混合的定义。$\blacksquare$
...
*证明*：$h_\mu(T) > 0$ 意味着存在有限划分 $\mathcal{P}$ 使 $h(T,\mathcal{P}) > 0$。由 Sinai 因子定理，这等价于系统包含非平凡的 Bernoulli 因子。任何 Bernoulli 移位是强混合的，因而是遍历的。更一般地，正熵蕴含系统的 Pinsker 因子非平凡，Kolmogorov 性质确保遍历性。$\blacksquare$
...
*证明概略*：由 Oseledets 乘法遍历定理，切空间分解为 Lyapunov 子空间 $\bigoplus E_i(x)$，增长率 $\lambda_i$。沿不稳定流形叶层，系统以速率 $\exp(\sum_{\lambda_i>0} \lambda_i)$ 膨胀，产生信息（熵）。Pesin 理论证明了不稳定流形与测度论熵的精确对应——每个不稳定叶层在小尺度上的膨胀对熵的贡献恰为 Lyapunov 指数。SRB 测度下，条件测度沿不稳定方向绝对连续于 Riemann 体积，实现熵的精确计算公式。$\blacksquare$
...
*证明*：由 Krylov-Bogolyubov 定理，$T$ 存在不变概率测度 $\mu$。取 $x$ 为 $\mu$ 的支撑中任一点。由 Poincaré 回复定理（保测变换），对 $\mu$-ae $x$，存在 $n_k \to \infty$ 使 $T^{n_k}x \to x$。因 $\mu$ 的支撑非空，必存在回复点。$\blacksquare$
...
*证明概略*：(1) $h_\mu(T) \leq h_{top}(T)$ 对所有 $\mu$ 成立：由熵的定义和生成覆盖的基数估计，测度论熵有拓扑熵为上界。(2) 上确界可达：对任意 $\varepsilon > 0$，取 $(n,\varepsilon)$-分离集 $E_n$，定义经验测度 $\mu_n = \frac{1}{|E_n|}\sum_{x\in E_n} \delta_x$，取弱*极限点 $\mu$。适当假设下熵上半连续，$h_\mu(T) \geq h_{top}(T)$，故上确界等于 $h_{top}(T)$。$\blacksquare$
...
*证明*：若 $\frac{\partial f}{\partial x}(0,0) \neq 0$，由隐函数定理，方程 $f(x,\mu)=0$ 在 $\mu=0$ 附近有唯一光滑解支 $x = g(\mu)$ 且 $g(0)=0$。此时平衡点数目和局部稳定性均不随 $\mu$ 变化，故 $\mu=0$ 不是分支点。因此分支的必要条件是 $\frac{\partial f}{\partial x}(0,0) = 0$。$\blacksquare$
...
*证明概略*：由中心流形定理将系统约化为二维，规范形 $\dot{z} = (\alpha(\mu)+i\omega(\mu))z + c(\mu)|z|^2 z + \cdots$。计算第一 Lyapunov 系数 $\ell_1(0) = \Re(c(0))/\omega(0)$（通过投影到中心流形并施行正规形变换）。$\alpha'(0)\ell_1(0) \neq 0$ 时横截条件和非退化条件成立，产生唯一分支族。$\ell_1(0) < 0$ 时极限环稳定（超临界），$> 0$ 不稳定（亚临界）。$\blacksquare$
...
*证明概略*：在鞍-焦点邻域构造 Poincaré 返回映射。由鞍点性质，轨道螺旋趋近平衡点后被不稳定方向带离。条件 $|\lambda_1| < \alpha$ 保证返回映射在远离平衡处为压缩。同宿轨道使返回映射在某区域有定义，其像为螺旋状条带。可分解为可数多连通分支，每分支包含 Smale 马蹄构造——由拉伸-折叠几何机制产生混沌不变集。$\blacksquare$
...
*证明概略*：由 Binet 第二公式 $\log\Gamma(z) = (z-\frac{1}{2})\log z - z + \frac{1}{2}\log(2\pi) + \int_0^\infty \frac{\varphi(t)}{t}e^{-zt}dt$（$\varphi(t) = \frac{1}{2} - \frac{1}{t} + \frac{1}{e^t-1}$）。被积函数 Taylor 展开 $e^{-zt}\varphi(t)/t = \sum_{k=1}^\infty \frac{B_{2k}}{2k(2k-1)} \frac{t^{2k-2}}{z^{2k-1}}e^{-zt}$ 逐项积分即得渐近级数。其中 $B_{2k}$ 为 Bernoulli 数。主项来自 Laplace 方法的鞍点近似。$\blacksquare$
...
*证明*：Bessel方程可写为 Sturm-Liouville形式 $(x J_\nu')' + (x - \nu^2/x)J_\nu = 0$。对 $\phi_n(x) = J_\nu(j_{\nu,n}x/a)$，$m \neq n$ 时利用分部积分和Sturm-Liouville理论：$(\lambda_m-\lambda_n)\int_0^a \phi_m\phi_n x dx = 0$，故正交。归一化常数由 $J_\nu$ 的递推关系 $\int_0^a J_\nu^2(j_{\nu,n}r/a) r dr = (a^2/2)J_\nu'(j_{\nu,n})^2$ 确定。$\blacksquare$
...
*证明概略*：Rodrigues公式 $P_n(x) = \frac{1}{w(x)K_n} \frac{d^n}{dx^n}[w(x)Q(x)^n]$ 统一了经典正交多项式的构造（$w$为权函数，$Q$为二次多项式）。所有经典族都是 $pF_q$ 的特例，如 Jacobi多项式 $P_n^{(\alpha,\beta)}(x) = \binom{n+\alpha}{n}{}_2F_1(-n,n+\alpha+\beta+1;\alpha+1;\frac{1-x}{2})$。Askey方案通过极限关系（如 $q \to 1$ 下 Askey-Wilson $\to$ Wilson $\to$ 经典多项式）组织层级。$\blacksquare$

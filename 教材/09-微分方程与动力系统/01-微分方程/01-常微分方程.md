...
**证明**：Picard迭代：$\phi_0(t)=x_0$，$\phi_{n+1}(t)=x_0+\int_{t_0}^t f(s,\phi_n(s))ds$。由Lipschitz条件，$\|\phi_{n+1}(t)-\phi_n(t)\|\le L\int_{t_0}^t\|\phi_n(s)-\phi_{n-1}(s)\|ds$。归纳得$\|\phi_{n+1}(t)-\phi_n(t)\|\le M L^n|t-t_0|^{n+1}/(n+1)!$，Weierstrass $M$-判别法给出$\phi_n$一致收敛于$\phi$。取极限验证$\phi$满足积分方程，Lipschitz连续性保证$\phi$为连续解。唯一性：若$\psi$也为解，$\|\phi(t)-\psi(t)\|\le L\int_{t_0}^t\|\phi(s)-\psi(s)\|ds$，由Gronwall不等式得$\|\phi-\psi\|=0$。$\blacksquare$

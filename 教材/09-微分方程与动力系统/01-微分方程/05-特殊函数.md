...
**证明思路**：利用 Euler 积分表示 $_2F_1(a,b;c;z) = \frac{\Gamma(c)}{\Gamma(b)\Gamma(c-b)} \int_0^1 t^{b-1} (1-t)^{c-b-1} (1-zt)^{-a} dt$。令 $z = 1$，化为 Beta 积分 $B(b, c-a-b)$，再用定理 97.2 转化为 Gamma 函数比值即得。$\blacksquare$

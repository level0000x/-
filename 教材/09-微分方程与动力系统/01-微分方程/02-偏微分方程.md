...
*证明概略*：核心是利用 Sobolev 不等式 $\|u\|_{L^{p^*}(\mathbb{R}^n)} \leq C \|\nabla u\|_{L^p(\mathbb{R}^n)}$（$p^* = np/(n-p)$，$1 \leq p < n$）。对全空间情形，利用位势算子 $u = I_1 * \nabla u$ 和 Hardy-Littlewood-Sobolev 不等式可证。对有界域 $\Omega$，通过延拓算子将 $W^{k,p}(\Omega)$ 嵌入到 $W^{k,p}(\mathbb{R}^n)$，再反复应用 Sobolev 不等式即得各情形。临界嵌入 $kp=n$ 情形需用 Trudinger-Moser 不等式。$\blacksquare$

**定理 93.3**（Rellich-Kondrachov 紧嵌入）：在上述条件下，若 $k p < n$ 且 $1 \leq q < p^*$（$p^* = np/(n - kp)$），则嵌入 $W^{k,p}(\Omega) \hookrightarrow L^q(\Omega)$ 是紧的。

*证明概略*：由 Sobolev 嵌入定理知嵌入是连续的。为证紧性，需证 $W^{k,p}(\Omega)$ 中任何有界序列有在 $L^q$ 中收敛的子列。核心工具是正则化（Friedrichs mollifier）和 Ascoli-Arzelà 定理。对全空间情形，用磨光得到一致紧支集的近似序列，再由平移的 $L^q$ 连续性（Kolmogorov-Riesz 紧性判据）抽取收敛子列。紧嵌入条件 $q < p^*$ 是关键的——临界指数 $q = p^*$ 时嵌入非紧。$\blacksquare$
...
*证明概略*：核心是通过差分商（difference quotient）方法建立内估计。令 $D_h u(x) = (u(x+he_i) - u(x))/h$。由弱形式 $\int \nabla u \cdot \nabla v = \int f v$，取 $v = -D_{-h} D_h u$，利用分部求和公式得 $\|D_h \nabla u\|_{L^2} \leq C\|f\|_{L^2}$。取 $h \to 0$ 得二阶弱导数有界，即 $u \in H^2$。迭代此过程得高阶正则性。边界正则性需利用边界拉平（flattening）技术。$\blacksquare$

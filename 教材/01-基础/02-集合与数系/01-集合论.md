...
外延公理是 ZFC 系统中唯一涉及等号的公理。在带有等号的一阶逻辑中，等号通常被视为逻辑符号，其基本性质（自反性、对称性、传递性和替换性）由逻辑公理提供。外延公理的作用是将集合的"相等"与"属于"关系联系起来：两个集合相等当且仅当它们有相同的元素。这一定义具有很强的还原论色彩——集合的全部"身份"仅由其元素决定。外延公理还可以等价地表述为：$\forall A\, \forall B\, (A \subseteq B \wedge B \subseteq A \rightarrow A = B)$，这一形式在证明中经常使用。外延公理排除了基于"内涵"来区分集合的可能性（见 3.1.3 节的讨论）。
...
并集公理允许我们将一个集合族中所有集合的"成员"收集到一个集合中。注意 $\bigcup \mathcal{F}$ 的记号：当 $\mathcal{F} = \{A, B\}$ 时，$\bigcup \mathcal{F} = A \cup B$ 就是通常的二元并集。当 $\mathcal{F} = \{A_1, A_2, \ldots\}$ 时，$\bigcup \mathcal{F} = \bigcup_{i=1}^{\infty} A_i$ 是可数并集。并集公理的关键在于它允许任意大的指标集——不仅限于有限集。这使得我们可以在 ZFC 中处理任意大的并集操作。在 Zermelo 1908 年的原始系统中，并集公理与分离公理（Aussonderungsaxiom）一起构成了主要的集合构造工具。

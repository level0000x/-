...
**定理 53.4.2（Mittag-Leffler 定理）**：给定 $\mathbb{C}$ 中离散点集 $\{z_n\}$ 和对应的主要部分 $P_n(z) = \sum_{k=1}^{m_n} \frac{c_{n,k}}{(z - z_n)^k}$，存在 $\mathbb{C}$ 上的亚纯函数以 $\{z_n\}$ 为极点集，且在 $z_n$ 处的主要部分恰为 $P_n(z)$。Mittag-Leffler 定理是 Weierstrass 因子分解定理的对偶，两者共同构成整函数和亚纯函数理论的基石。
...
其中 $N$ 为 $f$ 在 $\gamma$ 内部的零点个数（按重数计），$P$ 为极点个数（按重数计）。幅角原理的几何意义是：$\gamma$ 在映射 $f$ 下的像曲线绕原点 $N - P$ 圈。

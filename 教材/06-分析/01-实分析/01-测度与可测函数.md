...
**证明**：先证 $E$ 有界时的情形。由外测度定义，对任意 $\varepsilon > 0$，存在开矩体覆盖 $\{Q_j\}$ 使 $\sum |Q_j| < m(E) + \varepsilon/2$。取 $U = \bigcup_j Q_j^\circ$（$Q_j^\circ$ 为 $Q_j$ 的内部），则 $U$ 为开集，$E \subseteq U$ 且 $m(U) \leq \sum |Q_j^\circ| \leq \sum |Q_j| < m(E) + \varepsilon/2$。对有理多面体进行逼近，可得 $m(U) < m(E) + \varepsilon$。对内正则性，设 $E$ 有界可测，则其闭包 $\overline{E}$ 为紧集。对任意 $\varepsilon > 0$，由外正则性，存在开集 $U \supseteq \overline{E} \setminus E$ 使 $m(U) < \varepsilon$。令 $K = \overline{E} \setminus U$，则 $K$ 为紧集，$K \subseteq E$，且 $m(E \setminus K) = m((\overline{E} \setminus E) \cup (E \cap U)) \leq \varepsilon$。无界可测集 $E$ 可表为 $E = \bigcup_{n=1}^\infty (E \cap B_n(0))$（递增有界可测集），对每个 $E_n = E \cap B_n(0)$ 应用有界情形取 $K_n$，则 $m(E) = \lim m(K_n) = \sup_K m(K)$。$\blacksquare$

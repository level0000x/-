...
SPDE 的分类还涉及噪声的正则性。时空白噪声 $\xi$ 在 $d \geq 2$ 时是 Schwartz 分布而非函数，这导致不同的处理策略。对于 $d=1$ 的情形，时空白噪声可以与热半群卷积生成连续的随机场（随机卷积），从而可以沿用半线性 PDE 的框架。对于 $d=2$ 的情形，需要引入重正化（renormalization）——Hairer 的正则结构理论（2014，Fields 奖 2014）提供了系统处理此类奇异 SPDE 的代数框架，其核心思想是在局部描述中引入被截断的 Taylor 展开式的随机类比。对于 $d \geq 3$ 的情形，重正化可能不足以处理，需要更复杂的工具。SPDE 的另一分类维度是驱动噪声的类型：Gauss 白噪声（$\xi$ 为 Gauss 随机场）、Poisson 噪声（$\xi$ 为跳跃过程驱动）、分数 Brown 运动噪声（$\xi$ 为长程相关 Gauss 噪声，Hurst 指数 $H \neq 1/2$）。不同类型的噪声导致不同的正则性行为和物理应用场景——例如分数噪声在金融波动率建模和地下水流动建模中很常见。SPDE 理论在人口遗传学（Fleming-Viot 过程）、流界面动力学（KPZ 方程）、随机量化（$\Phi^4_d$ 模型）和金融数学（随机波动率 HJM 模型）等领域有广泛应用。

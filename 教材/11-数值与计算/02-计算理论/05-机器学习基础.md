...
**证明概要**：由对称化引理 $R(h)-\widehat{R}_n(h)\leq 2\mathcal{R}_n(\mathcal{H})+\sqrt{\log(1/\delta)/(2n)}$，其中 $\mathcal{R}_n$ 为Rademacher复杂度。由Massart有限类引理和Sauer-Shelah引理，$\mathcal{R}_n(\mathcal{H})\leq\sqrt{2d\log(en/d)/n}$，代入即得。$\blacksquare$
...
**证明概要**（Cybenko）：反证法。假设此类函数的闭包 $\mathcal{M}$ 不是整个 $C(I_n)$（$I_n=[0,1]^n$）。由Hahn-Banach定理，存在非零有界线性泛函 $L\in C(I_n)^*$（即 $I_n$ 上的正则Borel测度 $\mu$ 使得 $\int_{I_n}\sigma(\mathbf{w}^Tx+b)\,d\mu(x)=0$ 对所有 $\mathbf{w},b$）。由 $\sigma$ 的判别性质，可证 $\mu\equiv 0$，矛盾。$\blacksquare$

...
*证明概略*：（$\Rightarrow$）若 $T$ 有量词消去，$\exists x\varphi(x,\bar{c})$ 等价于无量词公式 $\psi(\bar{c})$。因 $\mathfrak{A}\models\psi(\bar{c})$ 且 $\mathfrak{C}\subseteq\mathfrak{A},\mathfrak{B}$ 共享 $\bar{c}$，故 $\mathfrak{B}\models\psi(\bar{c})$ 即 $\mathfrak{B}\models\exists x\varphi(x,\bar{c})$。（$\Leftarrow$）使用来回构造法（back-and-forth）证明任意两 $T$ 模型的初等等价，从而量化公式可消去为无量词公式的组合。$\blacksquare$
...
**证明**：（$\Rightarrow$）设 $T$ 模型完备，$\mathfrak{A}\subseteq\mathfrak{B}\models T$，则 $\mathfrak{A}\preceq\mathfrak{B}$。若 $\bar{a}\in A$ 且 $\mathfrak{B}\models\exists\bar{x}\varphi(\bar{x},\bar{a})$（$\varphi$ 无量词），由初等性 $\mathfrak{A}\models\exists\bar{x}\varphi(\bar{x},\bar{a})$，故存在 $\bar{c}\in A$ 满足 $\mathfrak{A}\models\varphi(\bar{c},\bar{a})$。因 $\varphi$ 无量词且 $\mathfrak{A}\subseteq\mathfrak{B}$，$\mathfrak{B}\models\varphi(\bar{c},\bar{a})$。故 $\bar{a}$ 在 $\mathfrak{A}$ 中已满足存在公式。（$\Leftarrow$）若条件成立，需证 $\mathfrak{A}\preceq\mathfrak{B}$ 对任意 $\mathfrak{A}\subseteq\mathfrak{B}\models T$。对公式复杂度归纳：原子公式平凡（子结构）；$\to,\neg$ 归纳直接；关键在 $\exists\bar{x}\psi$——将 $\psi$ 化为前束范式并逐个消去量词，存在量词由条件保证 $\mathfrak{A}$ 与 $\mathfrak{B}$ 一致，全称量词化为 $\neg\exists\neg$ 处理。$\blacksquare$
...
**证明**：（Turing $\to$ 部分递归）用 Gödel 编码将 TM 的带格局编码为自然数。定义函数 $g(e,x,t)$ 为第 $e$ 个 TM 在输入 $x$ 上运行 $t$ 步后的输出（若未停机则返回 0）。$g$ 是原始递归的。则 $f(x)\simeq U(\mu t[\text{第 }e\text{ 个 TM 在 }t\text{ 步时停机}])$，此为 $\mu$ 算子作用于原始递归谓词，故部分递归。（部分递归 $\to$ Turing）初等函数显然 Turing 可计算；复合对应子程序调用；原始递归对应循环；$\mu$ 算子对应穷举搜索（do-while 找到 $g(y)=0$）。所有构造可由 TM 编程实现。$\blacksquare$
...
*证明*：停机问题 $K$ 是 r.e. 的（模拟通用 Turing 机，若停机则接受），由定理 101.3 知 $K$ 非递归。而 $\mathbb{N}\setminus K$ 非 r.e.（否则 $K$ 递归）。$A$ 递归当且仅当 $A$ 和补集均为 r.e.：可计算 $\chi_A$ 通过并行枚举 $A$ 及其补集，其中恰有一个枚举成功。$\blacksquare$
